...
export default function VerHorariosDisponibles() {
  ...
  const fetchHorarios = async () => {
    try {
      const response = await axios.get(`${host}/horarios_disponibles`);
  ...
  const fetchProfesores = async () => {
    try {
      const response = await axios.get(`${host}/profesores`);
  ...
  const handleDelete = async (id) => {
    if (window.confirm('¿Estás seguro de que deseas eliminar este horario?')) {
      try {
        await axios.delete(`${host}/horarios_disponibles/${id}`); // Asumiendo que tienes el endpoint para eliminar
    ...
    <div className="min-h-screen bg-cover bg-center bg-no-repeat"
      ...
      <div className="relative min-h-screen flex flex-col items-center pt-32">
        <div className="w-full max-w-4xl p-6 bg-white bg-opacity-70 backdrop-filter backdrop-blur-lg rounded-lg shadow-md">
          <div>
            <Link to="/horarioDisponibles/crear" className="bg-green-600 text-white font-semibold px-6 py-3 rounded-lg hover:bg-green-700 transition-all duration-300">
